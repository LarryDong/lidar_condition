...
fprintf("-->Setting parallel pool... \n");
if isempty(gcp('nocreate')) 
	parpool(10);	% Start the parallel pool if it's not already started
end
fprintf("<-- Done. \n");


% Preallocate the array for storing condition numbers
cond_rr = zeros(total_scan, 1);
cond_tt = zeros(total_scan, 1);

fprintf("--> Begin the loop \n");
parfor i = 1:total_scan
	fprintf("  process: %d / %d, \n", i, total_scan);
    ptCloudMsg = msgs{i};
    ...
    for j = 1:numPoints
        ...
        [indices, ~] = findNearestNeighbors(ptCloud, ptCloud.Location(j, :), 5);
    ...
    cond_rr(i) = max(diag(S_rr)) / min(diag(S_rr));
    cond_tt(i) = max(diag(S_tt)) / min(diag(S_tt));
end
